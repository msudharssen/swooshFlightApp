...
async function flightInformation (data){
    ...
    try{
    let url = `https://aeroapi.flightaware.com/aeroapi/flights/${data}?start=${utcFormattedDate}&end=${utcFormattedDate2}`;
    let options = {
    method: 'GET',
    headers: {
    ...
    let localTimeOff = utcTime.setZone(`${result.flights[0].origin.timezone}`);
    
    let utcTimeString2 = result.flights[0].estimated_on;
    let utcTime2 = DateTime.fromISO(utcTimeString2, { zone: 'utc'});
    let localTimeOn = utcTime2.setZone(`${result.flights[0].destination.timezone}`);
    ...
    let info = [];
    info.push(result.flights[0].origin.code_iata);
    info.push(result.flights[0].origin.name);
    info.push(result.flights[0].origin.city);
    info.push(result.flights[0].destination.code_iata);
    info.push(result.flights[0].destination.name);
    info.push(result.flights[0].destination.city);
    info.push(result.flights[0].status);
    info.push(dateTakeOff);
    info.push(dateLand);
    info.push(convertedTimeOff);
    info.push(convertedTimeOn);
    info.push(result.flights[0].ident_iata);
    info.push(result.flights[0].progress_percent);
    info.push(result.flights[0].origin.timezone);
    info.push(result.flights[0].destination.timezone);
    return info;
}
catch(error){
    console.log(error);
