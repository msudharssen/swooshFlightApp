...
async function flightInformation (data){
    ...
    let localTimeOff = utcTime.setZone(`${result.flights[index].origin.timezone}`);
    
    let utcTimeString2 = result.flights[0].estimated_on;
    let utcTime2 = DateTime.fromISO(utcTimeString2, { zone: 'utc'});
    let localTimeOn = utcTime2.setZone(`${result.flights[index].destination.timezone}`);
    ...
    let info = [];
    info.push(result.flights[index].origin.code_iata);
    info.push(result.flights[index].origin.name);
    info.push(result.flights[index].origin.city);
    info.push(result.flights[index].destination.code_iata);
    info.push(result.flights[index].destination.name);
    info.push(result.flights[index].destination.city);
    info.push(result.flights[index].status);
    info.push(dateTakeOff);
    info.push(dateLand);
    info.push(convertedTimeOff);
    info.push(convertedTimeOn);
    info.push(result.flights[index].ident_iata);
    info.push(result.flights[index].progress_percent);
    info.push(result.flights[index].origin.timezone);
    info.push(result.flights[index].destination.timezone);
    return info;
}
catch(error){
    console.log(error);
